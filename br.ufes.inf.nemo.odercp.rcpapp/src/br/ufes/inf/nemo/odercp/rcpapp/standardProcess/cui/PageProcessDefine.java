...
public class PageProcessDefine {
		...
		for (int i = 0; i < subkprocesses.size(); i++) {
			if (subkprocesses.get(i).getkProcess().isEngineering()) {
				...
				combo.addSelectionListener(new SelectionListener() {
					...
					@Override
					public void widgetSelected(SelectionEvent e) {
						specificStandardProcess.getSpecialization().remove(
								processEngineering);
						processEngineering = hashKprocess.get(combo
								.getItem(combo.getSelectionIndex()));
						specificStandardProcess.getSpecialization().add(
								processEngineering);
					}
	...
	protected void createtree() {
		if (specificStandardProcess != null) {
			...
			if (!specificStandardProcess.getSpecialization().isEmpty()) {
				...
				Iterator<StandardProcess> itsubprocesses = specificStandardProcess
